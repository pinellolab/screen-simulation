...
def run_mageck(
    ...
    mageck_input_name = "sorted_reads/mageck_input_{}_{}.tsv".format(
        screen.get_id(),
        simulation_rep)

    if not os.path.exists(mageck_input_name) or rerun:
        _ = write_mageck_input(mageck_input_name, screen)
        
    mageck_res_name_prefix = "mageck_results/{}_{}".format(
        screen.get_id,
        simulation_rep
    )
    ...
    for r in range(iteration_per_screen):
        ...
        for sorting_bin in self.sorting_bins:
            ...
            try:
                ...
                return((None, None))
            mageck_res = self.target_info.merge(
                mageck_res, 
                right_on = "id", 
                left_on = "target_id"
                )
