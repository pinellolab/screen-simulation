...
def get_sorted_read_counts(cell_counts : pd.DataFrame, 
                          ...
                          sorting_bins = ["top", "bot", "bulk"]) -> pd.DataFrame:
    measures = ["guide", "target_edit", "reporter_edit"]
    read_counts = cell_counts[["target_id", "guide_num"]].reset_index(drop = True)
    for sorting_bin in sorting_bins:
        all_guide_count = cell_counts["{}_guide".format(sorting_bin)].sum()
        for measure in measures:
            bin_measure = "{}_{}".format(sorting_bin, measure)
            cell_counts["{}_p".format(sorting_bin)] = cell_counts[bin_measure] / all_guide_count
            assert not bin_measure in read_counts.columns
            read_counts[bin_measure] = \
                scipy.stats.binom.rvs(
                    n_read_sample, 
                    cell_counts["{}_p".format(sorting_bin)]
                )
    return(read_counts)
...
def simulate_reps(total_cells_per_rep = 10**5, 
    ...
    for rep in range(nreps):
        ...
        if sorting_mode == "topbot":
            ...
            sorting_bins = ["bot", "low", "mid", "high", "top"]
            ...
            read_counts = get_sorted_read_counts(
                cell_counts, 
                nreads_per_sample, 
                sorting_bins = sorting_bins + ["bulk"]
            )
        else:
